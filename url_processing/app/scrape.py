...
class DoScraping():
	...
	def classify_url(self):
		...
		try:
			...
			print("ERROR")
			# requests.post(os.getenv('LOGGER_ADDR'), params={'service':'url_processing', 'method':'POST', 'org':'NULL', 'org_input':'NULL', 'message':'URL Return object Not Found'})
			return json.dumps({"error":"URL is not valid"})
		...
		if urltype == 'html':
			...
			# requests.post(os.getenv('LOGGER_ADDR'), params={'service':'url_processing', 'method':'POST', 'org':'NULL', 'org_input':'NULL', 'message':'Error in processing URL'})
			return json.dumps({"error":"Error in processing URL"})
		elif urltype is None:
			# requests.post(os.getenv('LOGGER_ADDR'), params={'service':'url_processing', 'method':'POST', 'org':'NULL', 'org_input':'NULL', 'message':'URL Rejected : Unknow Type document'})
